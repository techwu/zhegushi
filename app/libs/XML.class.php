...
/*
	...
	$Id: XML.class.php,v 1.1 2009-12-14 09:02:46 wangxi Exp $
*/

function xml_unserialize(&$xml , $encode = 'utf-8' , $root = 'root') {
	$xml_parser = new XML($encode);
...
function xml_serialize(&$data, $htmlon = 0, $level = 1 , $fix = true , $iso = true) {
	...
	if(!empty($data)) {
		foreach($data as $key=>$val) {
			if(!is_array($val)) {
				...
				if(is_numeric($key)) {
					if($fix){
						$s .=  "$space<item_$key>".xml_serialize($val, $htmlon, $level+1 , $fix , $iso)."$space</item_$key>";
					}else{
						$s .=  "$space<item>".xml_serialize($val, $htmlon, $level+1 , $fix , $iso)."$space</item>";
					}
				} elseif($key === '') {
					$s .= '';
				} else {
					$s .= "$space<$key>".xml_serialize($val, $htmlon, $level+1 , $fix , $iso)."$space</$key>";
				}
			}
		}
	}
	$s = preg_replace("/([\x01-\x09\x0b-\x0c\x0e-\x1f])+/", ' ', $s);
	return $iso === true ? ($level == 1 ? "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?><root>" : '').$s.($level == 1 ? '</root>' : '') : ($level == 1 ? "<xml>" : '').$s.($level == 1 ? '</xml>' : '');
}
...
class XML {
	...
	function XML($encode = 'ISO-8859-1') {
		$this->parser = xml_parser_create($encode);
